                        ...
                        @if (is_array($category->image) && count($category->image) > 0)
                            @foreach (array_slice($category->image, 0, 4) as $image)
                                <div class="col-lg-3 col-md-4 col-sm-6 col-6 mb-4">
                                    <a href="{{ asset($image) }}" data-toggle="lightbox" data-gallery="category-gallery"
                                        class="d-block mb-4 h-100">
                                        <img src="{{ asset($image) }}" class="img-fluid img-thumbnail square-image"
                                            alt="Image Description">
                                    </a>
                                </div>
                            @endforeach
                            @foreach (array_slice($category->image, 4) as $image)
                                <div data-toggle="lightbox" data-gallery="category-gallery" data-src="{{ asset($image) }}"
                                    data-title="Hidden item"></div>
                            @endforeach
                        @else
